...
void trigg() {
	...
	rp_AcqSetTriggerDelay(-6144); //if there is noise at the end can use -6550 else -6144
...
void sendviatcp (char *tcpline, uint32_t buffer_size, float *buffer, int line_number) {
	int i=0;
	float tmp, scale=0.95;//1.5; //depend on maximum value of input, for test scale=1
